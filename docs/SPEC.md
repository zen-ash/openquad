...
## Part 12 - Google's 3D tiles (removed)

For a while the rest of downtown came from Google's Photorealistic 3D Tiles, streamed while
you play and lined up with our map (flattened with USGS ground heights, masked to OSM
footprints). I took them out in September 2026: it's our own buildings everywhere again.
The last version with them is the git tag `tiles-final`.

## Part 13 - a fence round the middle of campus

The whole map was walkable, but only the middle is really built. So there's a fence round
the main downtown campus: Hurt Park, the library, Langdale, Classroom South, the Student
Centers, the Sports Arena and the Research Tower.
...
- **Looks** - the whole map is drawn like before, the fence doesn't change that. Near the
  fence there's a bit of haze and a faint line on the ground, only when you're close.
